...
export function usePull() {
  ...
  const { mySocketId, initWs, roomLiving, liveUserList } = useWebsocket();
  ...
  function videoPlay(videoEl: HTMLVideoElement) {
    stopDrawingArr.value.forEach((cb) => cb());
    stopDrawingArr.value = [];
    if (appStore.videoControlsValue.renderMode === LiveRenderEnum.canvas) {
      ...
      appStore.videoControlsValue.renderMode === LiveRenderEnum.video
    ) {
      if (videoEl && videoWrapRef.value) {
        const rect = videoWrapRef.value.getBoundingClientRect();
  ...
  function handlePlay(data: ILiveRoom) {
    flvurl.value = [
      ...
      ? data.pull_cdn_flv_url!
      : data.pull_flv_url!;
    hlsurl.value = [
      LiveRoomTypeEnum.tencentcloud_css,
      LiveRoomTypeEnum.tencentcloud_css_pk,
    ].includes(data.type!)
      ? data.pull_cdn_hls_url!
      : data.pull_hls_url!;
    console.log('handlePlay', flvurl.value, appStore.liveLine);
