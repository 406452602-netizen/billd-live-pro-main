...
import { fetchLiveStartLive } from '@/api/live';
...
import { createVideo, generateBase64, handleAreaInfo } from '@/utils';
...
export function usePush() {
  ...
  async function startLive({
    ...
    const res = await fetchLiveStartLive({
      live_room_type: liveRoomType,
      area_id: handleAreaInfo(areas)?.[areas.length - 1]?.id,
      area_name: handleAreaInfo(areas)
        ?.map((v) => v.name)
        .join(' · '),
    });
