...
export function useQQLogin(data: { exp }) {
  ...
  const handleUrl = (state: string) =>
    `https://graph.qq.com/oauth2.0/authorize?response_type=code&client_id=${QQ_CLIENT_ID}&redirect_uri=${QQ_REDIRECT_URI}&scope=get_user_info,get_vip_info,get_vip_rich_info&state=${state}`;
...
export const useWechatLogin = (qrData: { platform; exp; loginId }) => {
  ...
  const url = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=${WECHAT_GZH_APPID}&redirect_uri=${redirectUri}&scope=snsapi_userinfo&response_type=code&state=${stateRes}`;
  hrefToTarget(url);
};
