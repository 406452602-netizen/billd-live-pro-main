...
import { WEBSOCKET_HOST, WEBSOCKET_PROTOCOL } from '@/spec-config';
...
export const useWebsocket = () => {
  ...
  function handleStartLive({
    ...
    if (appStore.liveRoomInfo) {
      appStore.liveRoomInfo.type = liveRoomType;
    }
    console.log(msrDelay, msrMaxDelay);
    ...
    if (liveRoomType === LiveRoomTypeEnum.srs) {
    ...
    } else if (liveRoomType === LiveRoomTypeEnum.tencentcloud_css) {
    ...
    } else if (liveRoomType === LiveRoomTypeEnum.tencentcloud_css_pk) {
      updateWebRtcTencentcloudCssConfig({
        isPk: true,
        roomId: roomId.value,
        canvasVideoStream: canvasVideoStream.value,
    ...
    ws.socketIo.on(
      ...
      async (data: WsOfferType['data']) => {
        ...
        if (
          ...
          data.live_room.type === LiveRoomTypeEnum.tencentcloud_css_pk
        ) {
          if (!route.query[URL_QUERY.pkKey]) {
            return;
          }
          if (data.receiver === mySocketId.value) {
            console.warn('是发给我的nativeWebRtcOffer-pk-tencentcloud_css_pk');
