 ...
 * 返回分区信息：[祖节点，父节点，子节点]
 */
export function handleAreaInfo(areas: IArea[]) {
  if (areas.length === 1) {
    return [areas[0]];
  }
  if (areas.length === 2) {
    const item1 = areas[0];
    const item2 = areas[1];
    if (item1.id === item2.p_id) {
      return [item1, item2];
    } else if (item1.p_id === item2.id) {
      return [item2, item1];
    } else {
      console.warn('没有对应分区关系');
      return [item1, item2];
    }
  } else {
    return areas;
...
export function findDataById({ rows, targetId }) {
  const targetData = rows.find((row) => row.id === targetId);
  const parentData = rows.find((row) => row.id === targetData.p_id);
  const childData = rows.filter((row) => row.p_id === targetId);
...
export const getObj = (obj: Record<string, any>, keyArr: string[]) => {
  const res: Record<string, any> = {};
  Object.keys(obj).forEach((item) => {
    if (keyArr.includes(item)) {
      res[item] = obj[item];
