  ...
  <div class="push-wrap">
              ...
              <div class="top-config">
                ...
                >
                  <div class="val">
                    <n-input-group>
                      <n-input
                        v-model:value="appStore.liveRoomInfo.title"
                        size="small"
    ...
    <div class="right">
      ...
      <div class="danmu-card">
        <div class="title">
          弹幕互动{{ liveUserCount ? `（${liveUserCount}）` : '' }}
        </div>
        <div class="list-wrap">
          <div
            ref="danmuListRef"
...
const { initWs, danmuList, sendDanmuTxt, sendDanmuImg } = useWebsocketIm();
...
async function initStartLiveInfo() {
  if (appStore.liveRoomInfo?.id) {
    const res = await fetchLiveLiveRoomIsLive(appStore.liveRoomInfo.id);
...
function roomNameIsOk() {
  ...
  if (!appStore.liveRoomInfo.title!.length) {
    window.$message.warning('请输入直播间标题！');
    return false;
  }
  if (
    appStore.liveRoomInfo.title!.length < 3 ||
    appStore.liveRoomInfo.title!.length > 20
  ) {
    window.$message.warning('直播间标题要求3-20个字符！');
    return false;
  }
  return true;
}
...
async function changeLiveRoomArea(v) {
  ...
  if (roomLiving.value) {
    ...
    if (res.code === 200) {
      ...
      if (appStore.liveRoomInfo?.areas) {
        appStore.liveRoomInfo.areas = selectArea.value;
      }
    }
  }
}
...
watch(
  () => appStore.treeAreaList,
  (newval) => {
    if (newval) {
      const res: any[] = [];
      appStore.treeAreaList.forEach((v) => {
...
onMounted(() => {
  worker.value = new Worker(
    NODE_ENV === 'production' ? 'dist/worker.js' : '/worker.js'
  );
  setTimeout(() => {
    scrollTo(0, 0);
  }, 100);
  console.log(roomId.value, '--');
  initWs({ roomId: roomId.value });
...
onUnmounted(() => {
  ...
  clearInterval(loopRtcIceConnectionStateTimer.value);
  recorder.value?.stop();
...
function handleSendGetLiveUser(liveRoomId: number) {
  clearInterval(loopGetLiveUserTimer.value);
  async function main() {
    const res = await fetchLiveRoomOnlineUserCount(liveRoomId);
    if (res.code === 200) {
      liveUserCount.value = res.data.count;
    }
  }
  setTimeout(() => {
    main();
...
async function uploadLivePreview() {
  ...
  const uploadRes = await useUpload({
    prefix:
      TENCENTCLOUD_COS['res-1305322458'].prefix['billd-live/client/msg-img/'],
    file,
  });
...
async function handleStartLive() {
  ...
  const res = await fetchLiveLiveRoomIsLive(Number(roomId.value));
  if (res.data) {
    window.$message.warning('当前正在开播');
    return;
  }
  initAudio();
  if (liveType === LiveRoomTypeEnum.msr) {
  ...
  startLive({
    ...
    liveRoomType: liveType,
    areas: selectArea.value,
    msrDelay: msrDelay.value,
    msrMaxDelay: 5000,
  });
  loopRtcIceConnectionState();
}

function loopRtcIceConnectionState() {
  clearInterval(loopRtcIceConnectionStateTimer.value);
  if ([LiveRoomTypeEnum.srs].includes(appStore.liveRoomInfo?.type!)) {
    loopRtcIceConnectionStateTimer.value = setInterval(() => {
      const res = networkStore.rtcMap.get('srs');
      if (res) {
        if (['disconnected'].includes(res.iceConnectionState || '')) {
  ...
  if (
    [LiveRoomTypeEnum.tencentcloud_css].includes(appStore.liveRoomInfo?.type!)
  ) {
    loopRtcIceConnectionStateTimer.value = setInterval(() => {
      const res = networkStore.rtcMap.get('tencentcloud_css');
      if (res) {
        if (['disconnected'].includes(res.iceConnectionState || '')) {
          roomLiving.value = false;
          clearInterval(loopRtcIceConnectionStateTimer.value);
