...
const ignoreIndexOf = [
  ...
  path.resolve(proDir, 'src/views/h5/room'),
  path.resolve(proDir, 'deploy/handleSyncCodeup.mjs'),
  path.resolve(proDir, 'deploy/handleSyncGitee.mjs'),
  path.resolve(proDir, 'deploy/handleSyncDoc.mjs'),
...
if (process.cwd().indexOf('galaxy-s10') !== -1) {
  ...
  clearOld().then(() => {
    ...
    proPkg.name = 'billd-live';
    proPkg.repository.url = 'https://github.com/galaxy-s10/billd-live';
    proPkg.bugs.url = 'https://github.com/galaxy-s10/billd-live/issues';
    ...
    try {
      execSync(`git rm -r -f --cached .`, { cwd: freeDir });
    } catch (error) {
      console.log(error);
    }
    execSync(`git add .`, { cwd: freeDir });
    let commitMsg = 'fix: 修复已知问题';
    try {
      commitMsg = execSync('git show -s --format=%s').toString().trim();
    } catch (error) {
      console.log(error);
    }
